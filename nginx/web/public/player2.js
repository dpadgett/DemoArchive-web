...
{
  player['verified'] = ('name' in player);
  if (!('name' in player)) {
  ...
  if (player['verified']) {
    cells[0] += " <img src=\"verified.jpg\" height=\"15\" width=\"15\" style=\"vertical-align:text-bottom\"/>";
  }
  ...
  checkbox.attr('name', player['_id']['$oid']);
  checkbox.click(updateMergeButton);
  var playerLinkCell = $('<td>');
  var playerLink = $('<a>Link</a>');
  playerLink.attr('href', 'debugplayer2.html#id=' + player['_id']['$oid']);
  console.log(player);
